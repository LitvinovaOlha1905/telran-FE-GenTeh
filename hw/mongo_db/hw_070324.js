...
db.products.updateMany(
	{},
	{ $addToSet: { country: { $each: ["Франция", "Италия"] } } }
);

// 14. Для товаров с id 1, 3 и 6 удалить поставщика Италия
db.products.updateMany({ _id: { $in: [1, 3, 6] } }, { $pop: { country: 1 } });
// или
db.products.updateMany(
	{ _id: { $in: [1, 3, 6] } },
	{ $pull: { country: "Италия" } }
